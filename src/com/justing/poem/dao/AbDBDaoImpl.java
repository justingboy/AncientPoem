...
import com.justing.poem.annotation.ActionType;
import com.justing.poem.annotation.Column;
import com.justing.poem.annotation.Id;
import com.justing.poem.annotation.Relations;
import com.justing.poem.annotation.RelationsType;
import com.justing.poem.annotation.Table;
import com.justing.poem.db.AbBasicDBDao;
import com.justing.poem.orm.AbTableHelper;
import com.justing.poem.utils.AbLogUtil;
import com.justing.poem.utils.AbStrUtil;
...
public class AbDBDaoImpl<T> extends AbBasicDBDao implements AbDBDao<T> {
	...
	public AbDBDaoImpl(SQLiteOpenHelper dbHelper, Class<T> clazz) {
		...
		// 加载所有字段
		this.allFields = AbTableHelper.joinFields(this.clazz
				.getDeclaredFields(), this.clazz.getSuperclass()
				.getDeclaredFields());
		...
		AbLogUtil.d(AbDBDaoImpl.class, "clazz:" + this.clazz + " tableName:"
	...
	public T queryOne(int id) {
		synchronized (lock) {
			...
			AbLogUtil.d(AbDBDaoImpl.class, "[queryOne]: select * from "
					+ this.tableName + " where " + this.idColumn + " = '" + id
					+ "'");
			List<T> list = queryList(null, selection, selectionArgs, null,
					null, null, null);
	 ...
	 * 
	 * @param sql
	 *            完整的sql如：select * from a ,b where a.id=b.id and a.id = ?
	 * @param selectionArgs
	 *            绑定变量值
	 * @param clazz
	 *            返回的对象类型
	...
	public List<T> rawQuery(String sql, String[] selectionArgs, Class<T> clazz) {
		...
		try {
			lock.lock();
			checkDBOpened();
			AbLogUtil.d(AbDBDaoImpl.class,
					"[rawQuery]: " + getLogSql(sql, selectionArgs));
	...
	public boolean isExist(String sql, String[] selectionArgs) {
		...
		try {
			...
			checkDBOpened();
			AbLogUtil.d(AbDBDaoImpl.class,
					"[isExist]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
	...
	public List<T> queryList(String[] columns, String selection,
		...
		try {
			lock.lock();
			checkDBOpened();
			AbLogUtil.d(AbDBDaoImpl.class, "[queryList] from " + this.tableName
			...
			for (Field relationsField : allFields) {
				...
				Relations relations = (Relations) relationsField
						.getAnnotation(Relations.class);
				// 获取外键列名
				...
				for (T entity : list) {
					...
					if (RelationsType.one2one.equals(type)) {
						...
						if (relationsField.getType().isAnnotationPresent(
								Table.class)) {
							Table table = (Table) relationsField.getType()
									.getAnnotation(Table.class);
							relationsTableName = table.name();
						}

						List<T> relationsList = new ArrayList<T>();
						Field[] relationsEntityFields = relationsField
								.getType().getDeclaredFields();
						for (Field relationsEntityField : relationsEntityFields) {
							Column relationsEntityColumn = (Column) relationsEntityField
									.getAnnotation(Column.class);
							// 获取外键的值作为关联表的查询条件
							if (relationsEntityColumn != null
									&& relationsEntityColumn.name().equals(
											foreignKey)) {
								...
								for (Field entityField : allFields) {
									...
									Column entityForeignKeyColumn = (Column) entityField
											.getAnnotation(Column.class);
									if (entityForeignKeyColumn == null) {
										continue;
									}
									if (entityForeignKeyColumn.name().equals(
											foreignKey)) {
										value = String.valueOf(entityField
												.get(entity));
										break;
									}
								}
								// 查询数据设置给这个域
								cursor = db.query(relationsTableName, null,
										foreignKey + " = ?",
										new String[] { value }, null, null,
										null, null);
								getListFromCursor(relationsField.getType(),
										relationsList, cursor);
								if (relationsList.size() > 0) {
									// 获取关联表的对象设置值
									relationsField.set(entity,
											relationsList.get(0));
								}
					...
					} else if (RelationsType.one2many.equals(type)
							|| RelationsType.many2many.equals(type)) {
						// 一对多关系
						...
						if (listEntityClazz.isAnnotationPresent(Table.class)) {
							Table table = (Table) listEntityClazz
									.getAnnotation(Table.class);
							relationsTableName = table.name();
						}

						List<T> relationsList = new ArrayList<T>();
						Field[] relationsEntityFields = listEntityClazz
								.getDeclaredFields();
						for (Field relationsEntityField : relationsEntityFields) {
							Column relationsEntityColumn = (Column) relationsEntityField
									.getAnnotation(Column.class);
							...
							if (relationsEntityColumn != null
								...
								cursor = db.query(relationsTableName, null,
										...
										new String[] { value }, null, null,
										null, null);
								getListFromCursor(listEntityClazz,
										relationsList, cursor);
								if (relationsList.size() > 0) {
									// 获取关联表的对象设置值
	...
	private void getListFromCursor(Class<?> clazz, List<T> list, Cursor cursor)
		...
		while (cursor.moveToNext()) {
			...
			// 加载所有字段
			List<Field> allFields = AbTableHelper.joinFields(entity.getClass()
					.getDeclaredFields(), entity.getClass().getSuperclass()
					.getDeclaredFields());
	...
	public long insert(T entity, boolean flag) {
		...
		try {
			...
			AbLogUtil.d(AbDBDaoImpl.class, "[insert]: insert into "
					+ this.tableName + " " + sql);
			...
			for (Field relationsField : allFields) {
				...
				if (RelationsType.one2one.equals(type)) {
					...
					if (relationsEntity != null) {
						...
						if (flag) {
							// id自增
							sql = setContentValues(relationsEntity,
									relationsCv, TYPE_INCREMENT, METHOD_INSERT);
						} else {
							// id需指定
							sql = setContentValues(relationsEntity,
									relationsCv, TYPE_NOT_INCREMENT,
									METHOD_INSERT);
						}
						String relationsTableName = "";
						if (relationsEntity.getClass().isAnnotationPresent(
								Table.class)) {
							Table table = (Table) relationsEntity.getClass()
									.getAnnotation(Table.class);
							relationsTableName = table.name();
						}

						AbLogUtil.d(AbDBDaoImpl.class, "[insert]: insert into "
								+ relationsTableName + " " + sql);
					...
					if (list != null && list.size() > 0) {
						for (T relationsEntity : list) {
							...
							if (relationsEntity.getClass().isAnnotationPresent(
									Table.class)) {
								Table table = (Table) relationsEntity
	...
	public long[] insertList(List<T> entityList, boolean flag) {
		...
		try {
			...
			for (int i = 0; i < entityList.size(); i++) {
				...
				AbLogUtil.d(AbDBDaoImpl.class, "[insertList]: insert into "
						+ this.tableName + " " + sql);
	...
	public int delete(int id) {
		...
		try {
			...
			AbLogUtil.d(AbDBDaoImpl.class,
					"[delete]: delelte from " + this.tableName + " where "
	...
	public int delete(String whereClause, String[] whereArgs) {
		...
		try {
			...
			AbLogUtil.d(AbDBDaoImpl.class, "[delete]: delete from "
					+ this.tableName + mLogSql);
	...
	public int update(T entity) {
		...
		try {
			...
			AbLogUtil.d(
					AbDBDaoImpl.class,
					"[update]: update " + this.tableName + " set " + sql
							+ " where "
							+ where.replace("?", String.valueOf(id)));
	...
	public List<Map<String, String>> queryMapList(String sql,
		...
		try {
			...
			checkDBOpened();
			AbLogUtil.d(AbDBDaoImpl.class,
					"[queryMapList]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			while (cursor.moveToNext()) {
	...
	public int queryCount(String sql, String[] selectionArgs) {
		...
		try {
			...
			checkDBOpened();
			AbLogUtil.d(AbDBDaoImpl.class,
					"[queryCount]: " + getLogSql(sql, selectionArgs));
			cursor = db.query(this.tableName, null, sql, selectionArgs, null,
					null, null);
			if (cursor != null) {
	...
	public void execSql(String sql, Object[] selectionArgs) {
		try {
			...
			checkDBOpened();
			AbLogUtil.d(AbDBDaoImpl.class,
					"[execSql]: " + getLogSql(sql, selectionArgs));
	...
	private void checkDBOpened() {
		if (db == null) {
			throw new RuntimeException(
					"先调用 startReadableDatabase()或者startWritableDatabase(boolean transaction)初始化数据库。");
		}
	}
